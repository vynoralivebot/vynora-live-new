...
def get_booking_plans():
    # Ignore legacy 15/20/25-minute settings from older deployments.
    if db is None:
        ...
        if isinstance(plans, list) and plans:
            ...
            for p in plans:
                if not isinstance(p, dict):
                    continue
                minutes=int(p.get("minutes",0)); tokens=int(p.get("tokens",0))
                if minutes in (1,2,5,10,30) and tokens>0:
                    item=dict(p); item["minutes"]=minutes; item["tokens"]=tokens
                    item.setdefault("rupees",tokens); item.setdefault("demo",minutes==1)
                    normalized.append(item)
            if {int(x["minutes"]) for x in normalized} == {1,2,5,10,30}:
                return sorted(normalized,key=lambda x:int(x["minutes"]))
    except Exception:
        pass
    return BOOKING_PLANS
...
def host_profile(host_id: int):
    ...
    return {"user_id":host_id,"name":h.get("name") or u.get("name","Host"),"username":h.get("username") or u.get("username",""),"photo_url":public_photo_url(host_id, u) or h.get("photo_url",""),"online":bool(h.get("online")),"country":h.get("country","IN"),"country_name":h.get("country_name","India"),"demo":bool(h.get("demo",False)),"total_tokens":int(h.get("total_tokens",0)),"available_earnings":float(h.get("available_earnings",0)),"gift_tokens":float(h.get("gift_tokens",0)),"host_share":float(h.get("host_share",HOST_SHARE)),"filter_name":h.get("filter_name","natural"),"status":h.get("status")}

@app.post("/api/host/apply")
def host_apply(data: HostApplyModel):
    require_user(data.user_id)
    existing = host_doc(data.user_id)
    d = {"user_id":uid(data.user_id),"name":data.name,"username":data.username,"phone":data.phone,"age":data.age,"bio":data.bio,"status":"pending","online":False,"country":"IN","country_name":"India","total_tokens":0,"available_earnings":0,"host_share":HOST_SHARE,"created_at":now_ts(),"updated_at":now_ts()}
...
def book_slot(data: BookingModel):
    ...
    plan=next((p for p in get_booking_plans() if int(p.get("minutes",0))==int(data.minutes)),None)
    if not plan: raise HTTPException(400,"Invalid duration")
    plan_minutes=int(plan["minutes"]); plan_tokens=int(plan.get("tokens",plan.get("rupees",0)))
    is_demo=bool(plan.get("demo",False)) or plan_minutes==1
    ...
    u=user_doc(data.user_id) or ensure_user(data.user_id)[0]
    user_country=(u.get("country") or "IN").upper(); host_country=(h.get("country") or "IN").upper()
    if not bool(h.get("online")): raise HTTPException(409,"Host is currently offline. Please choose an online Host or try again later.")
    if host_country != user_country:
        ...
        notify_user(data.user_id,message); raise HTTPException(403,"You cannot book a Host from another country")
    ...
    busy=overlap(data.host_id,start,end); bid=str(uuid.uuid4()); now=now_ts()
    if is_demo:
        reserved=col("users").find_one_and_update(
            {"user_id":uid(data.user_id),"tokens":{"$gte":plan_tokens},"demo_used":{"$ne":True}},
            {"$inc":{"tokens":-plan_tokens},"$set":{"demo_used":True,"demo_used_at":now,"demo_booking_id":bid,"updated_at":now}},
            return_document=ReturnDocument.AFTER)
    else:
        reserved=col("users").find_one_and_update(
            {"user_id":uid(data.user_id),"tokens":{"$gte":plan_tokens}},
            {"$inc":{"tokens":-plan_tokens},"$set":{"updated_at":now}},
            return_document=ReturnDocument.AFTER)
    if not reserved:
        raise HTTPException(409 if is_demo else 400,"Demo already used or insufficient tokens." if is_demo else "Insufficient tokens")
    doc={"booking_id":bid,"user_id":uid(data.user_id),"host_id":uid(data.host_id),"plan_id":plan.get("id",f"{plan_minutes}min"),"plan_name":plan.get("name",f"{plan_minutes} Minutes"),"minutes":plan_minutes,"rupees":int(plan.get("rupees",plan_tokens)),"tokens":plan_tokens,"is_demo":is_demo,"requested_start":start,"scheduled_start":None,"scheduled_end":None,"status":"pending","busy_at_request":busy,"created_at":now,"updated_at":now,"call_started_at":None,"call_ended_at":None,"user_joined_at":None,"host_joined_at":None,"earnings_credited":False}
    try:
        col("bookings").insert_one(doc)
    except Exception:
        col("users").update_one({"user_id":uid(data.user_id)},{"$inc":{"tokens":plan_tokens}})
        if is_demo:
            col("users").update_one({"user_id":uid(data.user_id),"demo_booking_id":bid},{"$set":{"demo_used":False,"demo_used_at":None,"demo_booking_id":None,"updated_at":now_ts()}})
        raise HTTPException(500,"Booking could not be created. Your Coins were restored.")
    notify_user(data.user_id,f"✅ <b>Booking request submitted</b>\n\nHost: {h.get('name','Host')}\nPackage: {plan.get('name',f'{plan_minutes} Minutes')}\nDuration: {plan_minutes} min\nCharge: {plan_tokens} Coins\n\nHost confirmation ka wait karein.")
    ...
    notify_request(f"📅 <b>NEW BOOKING REQUEST</b>\n\nUser: <code>{data.user_id}</code>\nHost: {h.get('name','Host')}\nPackage: {plan.get('name',f'{plan_minutes} Minutes')}\nDuration: {plan_minutes} min\nCoins: {plan_tokens}\nDemo: {'YES' if is_demo else 'NO'}\nRequested: {iso(start)}\nBusy: {'YES' if busy else 'NO'}",[[{"text":"✅ Accept","callback_data":f"accept_booking:{bid}"},{"text":"❌ Reject","callback_data":f"reject_booking:{bid}"}]])
    notify_full(f"📅 NEW BOOKING\nBooking: <code>{bid}</code>\nUser: {data.user_id}\nHost: {data.host_id}\nDuration: {plan_minutes} min\nCoins: {plan_tokens}\nDemo: {'YES' if is_demo else 'NO'}\nRequested: {iso(start)}\nBusy: {busy}")
    return {"status":"success","booking_id":bid,"busy":busy,"is_demo":is_demo,"plan":{"id":plan.get("id"),"name":plan.get("name"),"minutes":plan_minutes,"rupees":int(plan.get("rupees",plan_tokens)),"tokens":plan_tokens},"message":"Booking submitted"}
...
def admin_command(chat_id, from_id, text):
    ...
    try:
        if cmd in ("/helpadmin","/adminhelp"):
            ...
            target=int(args[0]); ensure_user(target); col("hosts").update_one({"user_id":target},{"$set":{"user_id":target,"status":"approved","online":False,"updated_at":now_ts()},"$setOnInsert":{"total_tokens":0,"available_earnings":0,"gift_tokens":0,"filter_name":"natural","host_share":HOST_SHARE}},upsert=True); tg_send(chat_id,f"✅ Host approved: <code>{target}</code>"); notify_user(target,"🎙️ आपका Host account approve हो गया है। अब आप bookings receive कर सकते हैं।"); notify_full(f"🎙️ HOST APPROVED\nAdmin: {from_id}\nHost: {target}\nTeam Group: Host approved and ready for monitoring")
            ...
            if minutes not in (1,2,5,10,30) or tokens<1: raise ValueError("Use: /setplan MINUTES TOKENS")
            plans=get_booking_plans(); found=False
            for x in plans:
                if int(x["minutes"])==minutes: x["tokens"]=tokens; x["rupees"]=tokens; found=True
            if not found: raise ValueError("Package not found")
        ...
        elif cmd=="/sethostshare" and len(args)>=2:
            target=int(args[0]); percent=float(args[1])
            if percent<0 or percent>100: raise ValueError("Share must be 0-100")
            if not host_doc(target): raise ValueError("Host not found")
            col("hosts").update_one({"user_id":target},{"$set":{"host_share":percent/100.0,"updated_at":now_ts()}})
            tg_send(chat_id,f"✅ Host <code>{target}</code> share set to <b>{percent:.2f}%</b>")
            notify_full(f"⚙️ HOST SHARE UPDATED\nAdmin: {from_id}\nHost: {target}\nShare: {percent:.2f}%")
        elif cmd=="/hostrate" and args:
            target=int(args[0]); h=host_doc(target)
            if not h: raise ValueError("Host not found")
            tg_send(chat_id,f"🎙️ Host <code>{target}</code>\nName: {h.get('name','Host')}\nHost Share: {float(h.get('host_share',HOST_SHARE))*100:.2f}%\nAvailable Earnings: ₹{float(h.get('available_earnings',0)):.2f}")
    ...
    if "callback_query" in upd:
        ...
        try:
            ...
            if action in ("approve_host","reject_host") and from_id in ADMIN_IDS:
                ...
                col("hosts").update_one({"user_id":target},{"$set":{"user_id":target,"status":status,"online":False,"updated_at":now_ts()},"$setOnInsert":{"total_tokens":0,"available_earnings":0,"gift_tokens":0,"filter_name":"natural","host_share":HOST_SHARE}},upsert=True)
...
def finalize_call(booking_id, ended_at=None, reason="completed"):
    ...
    end=int(ended_at or now_ts()); start=int(b.get("call_started_at") or end); actual=max(0,end-start)
    booked_tokens=int(b.get("tokens",0)); booked_minutes=max(1,int(b.get("minutes",1)))
    if actual<=0:
        charged=0
    else:
        connected_minutes=max(1,(actual+59)//60)
        charged=min(booked_tokens,max(1,int((booked_tokens*connected_minutes+booked_minutes-1)//booked_minutes)))
    refund=max(0,booked_tokens-charged)
    host=host_doc(b["host_id"]) or {}; host_share=max(0.0,min(1.0,float(host.get("host_share",HOST_SHARE))))
    host_earned=round(charged*host_share,2); platform_earned=round(charged-host_earned,2)
    fresh=col("bookings").find_one_and_update({"booking_id":booking_id,"earnings_credited":{"$ne":True}},{"$set":{"status":"completed","call_ended_at":end,"actual_seconds":actual,"charged_tokens":charged,"refunded_tokens":refund,"host_share":host_share,"host_earned":host_earned,"platform_earned":platform_earned,"earnings_credited":True,"completion_reason":reason,"updated_at":end}},return_document=ReturnDocument.AFTER)
    if not fresh: return b
    if refund: col("users").update_one({"user_id":b["user_id"]},{"$inc":{"tokens":refund}})
    col("hosts").update_one({"user_id":b["host_id"]},{"$inc":{"total_tokens":host_earned,"available_earnings":host_earned},"$set":{"updated_at":end}})
    report=(f"📊 <b>1v1 CALL COMPLETED</b>\n\nBooking: <code>{b['booking_id']}</code>\nUser: {b['user_id']}\nHost: {b['host_id']}\nPackage: {b.get('plan_name',f"{b['minutes']} Minutes")}\nBooked: {b['minutes']} min / {booked_tokens} Coins\nActual Connected: {round(actual/60,2)} min\nCharged: {charged} Coins\nRefunded: {refund} Coins\nHost Share: {host_share*100:.0f}% = ₹{host_earned:.2f}\nPlatform: ₹{platform_earned:.2f}\nStart: {iso(start)}\nEnd: {iso(end)}")
    notify_request(report); notify_full(report)
    ...
    pending=col("bookings").find({"host_id":b["host_id"],"status":"pending","busy_at_request":True}); host_name=(host_doc(b["host_id"]) or {}).get("name","Host")
    for pb in pending: notify_user(pb["user_id"],f"🟢 <b>{host_name} is available again</b>\nYour booking request is still waiting for host confirmation.")
    return fresh
...
def startup():
    ...
    if db is not None:
        try:
            col("users").update_many({"demo_used":{"$exists":False}},{"$set":{"demo_used":False,"demo_used_at":None,"demo_booking_id":None}})
            col("hosts").update_many({"host_share":{"$exists":False}},{"$set":{"host_share":HOST_SHARE}})
